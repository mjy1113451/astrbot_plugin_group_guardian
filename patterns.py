...
AD_PATTERNS = [
    ...
    r'(?:[进加入]\s*\d{5,12}\s*(?:群|裙)\s*.{0,8}(?:外挂|辅助|科技|挂|脚本|插件|软件|资源|福利|视频|资料|教程|cdk|激活码))',
    # 进/加/入 + 群/裙 + QQ群号(5-12位) + (可选: 领/拿/取/获取/免费) + 外挂/辅助/科技/...
    r'(?:[进加入]\s*(?:群|裙)\s*\d{5,12}\s*.{0,8}(?:外挂|辅助|科技|挂|脚本|插件|软件|资源|福利|视频|资料|教程|cdk|激活码))',
    # 进/加/入 + QQ群号(5-12位) + 群/裙 + 领/拿/取/获取/白嫖/免费/送/赠 (无后续物品也判定为广告)
    r'(?:[进加入]\s*\d{5,12}\s*(?:群|裙)\s*[领拿取获白免送赠])',
    # 群号 + 群 + 领/拿/取... + 外挂/辅助/科技/...（不强制要求"进"开头）
    r'(?:\d{5,12}\s*(?:群|裙)\s*.{0,5}(?:领|拿|取|获取|白嫖|免费|送|赠)\s*.{0,5}(?:外挂|辅助|科技|挂|脚本|插件|软件|资源|福利|视频|资料|教程|cdk|激活码))',
    # 领/拿/取/获取/白嫖/免费 + (可选副词) + 外挂/辅助/科技/挂 (单独一句话表达"领免费辅助"也算)
    r'(?:(?:领|拿|取|获取|白嫖|免费|赠送|送)\s*.{0,3}(?:外挂|辅助|科技|挂|脚本|插件|破解|cdk|激活码))',
    # === 既有规则保留 ===
