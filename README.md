...
> 当前版本：**v1.9.4** | 最低 AstrBot 版本：**>=4.24.2**
...
| 计划功能 | 说明 | 预计版本 |
| -------- | ---- | -------- |
| **SQLite 数据库存储** | 审核日志迁移至 SQLite，支持万级日志高效查询、按时间/群号/用户ID索引检索、长期历史数据保留，替代当前 JSON 文件方案 | v2.0.0 |
| **GIF 帧级拆分审核** | 使用 Pillow 将 GIF 拆分为关键帧，逐帧送视觉模型审核，彻底解决动图中间帧违规内容遗漏 | v2.0.0 |
| **审核规则热更新** | 支持通过 WebUI 动态添加/修改/删除正则规则和词库关键词，无需重启插件 | v2.1.0 |
