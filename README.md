...
> 当前版本：**v2.2.6** | 最低 AstrBot 版本：**>=4.24.2**
...
| 功能 | 说明 |
|------|------|
| 分类不存在提示 | 关键词分类输入不存在时前端就地提示，避免空白误解 |
| 分页增强 | 规则/关键词列表增加首页/末页操作与边界保护 |
| 导入结果详情弹窗 | 批量导入后支持查看重复词完整清单并一键导出 |

### 审核能力增强
| 功能 | 说明 |
|------|------|
| 审核规则优先级 | 支持规则优先级排序，先命中高优先级再执行动作 |
| 命中样本回放 | 在日志详情中回放命中规则与命中词，便于调参 |

### 稳定性与运维
| 功能 | 说明 |
|------|------|
| 热更新压测脚本 | 提供导入/编辑/删除回归脚本，验证大词库场景稳定性 |
| 重建状态历史 | 保存最近重建记录（开始/完成/失败），便于排查 |
