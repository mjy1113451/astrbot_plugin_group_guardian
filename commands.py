...
class CommandsMixin:
    ...
    # 注意：请在 Main 类中添加以下初始化逻辑（如果尚未存在）：
    # self.custom_swear_file = os.path.join(os.path.dirname(__file__), "custom_swear_words.json")
    # self._load_custom_swear_words()  # 加载用户自定义脏话
                ...
                if keyword.lower() in text.lower():
                    ...
                    if search_type != "all":
                        ...
                        if search_type == "swear":
                            # 优先使用 _swear_matcher（如果已初始化），否则回退到简单的关键词包含检查
                            if hasattr(self, '_swear_matcher') and self._swear_matcher:
                                is_match = self._swear_matcher.is_match(text)
                            else:
                                # 简单回退：检查文本是否包含关键词（适配无AC自动机环境）
    ...
    async def cmd_add_swear(self, event: AstrMessageEvent):
        '''添加自定义脏话关键词。用法: /添加脏话 <关键词>'''
        if not await self._is_plugin_admin(event):
            yield event.plain_result("仅插件管理员可以添加脏话关键词")
            return
        
        args = event.message_str.split(maxsplit=1)
        if len(args) < 2:
            yield event.plain_result("用法: /添加脏话 <关键词>\n示例: /添加脏话 笨蛋")
            return
        ...
        # 确保列表和集合已初始化
        if not hasattr(self, 'custom_swear_words'):
            self.custom_swear_words = []
        if not hasattr(self, 'custom_swear_set'):
            self.custom_swear_set = set()

        if word in self.custom_swear_set:
            yield event.plain_result(f"关键词「{word}」已存在于自定义词库中")
            return

        # 更新内存数据
        self.custom_swear_words.append(word)
        self.custom_swear_set.add(word)
        
        # 更新 AC 自动机（如果存在）
        if hasattr(self, '_swear_matcher') and self._swear_matcher:
            try:
                self._swear_matcher.add_word(word)
            except Exception as e:
                logger.warning(f"添加关键词到AC自动机失败: {e}")

        # 持久化保存到文件
        self._save_custom_swear_words()
        
        yield event.plain_result(f"已添加脏话关键词: {word}")

    async def cmd_del_swear(self, event: AstrMessageEvent):
        '''删除自定义脏话关键词。用法: /删除脏话 <关键词>'''
        if not await self._is_plugin_admin(event):
            yield event.plain_result("仅插件管理员可以删除脏话关键词")
            return
            
        args = event.message_str.split(maxsplit=1)
        if len(args) < 2:
            yield event.plain_result("用法: /删除脏话 <关键词>")
            return
        ...
        if word not in self.custom_swear_set:
            yield event.plain_result(f"关键词「{word}」不在自定义词库中")
            return

        # 从内存中移除
        if word in self.custom_swear_words:
            self.custom_swear_words.remove(word)
        self.custom_swear_set.discard(word)
        
        # 注意：部分简单的 AC 自动机实现可能不支持动态删除词，
        # 如需彻底生效可能需要重新构建整个 Matcher，这里暂仅从列表移除，
        # 重启后加载时不会加载已删除的词。
        # 若 Matcher 支持 remove_word，可在此调用。

        # 持久化保存
        self._save_custom_swear_words()
        
        yield event.plain_result(f"已删除脏话关键词: {word}")

    async def cmd_list_swear(self, event: AstrMessageEvent):
        '''查看自定义脏话关键词列表。用法: /查看脏话'''
        if not await self._is_plugin_admin(event):
            yield event.plain_result("仅插件管理员可以查看脏话关键词")
            return
            
        words = getattr(self, 'custom_swear_words', [])
        if not words:
            yield event.plain_result("当前无自定义脏话关键词")
        else:
            # 防止刷屏，每行显示5个
            chunk_size = 10
            lines = []
            ...
            yield event.plain_result(f"自定义脏话关键词列表 ({len(words)}个):\n" + "\n".join(lines))

    def _save_custom_swear_words(self):
        """保存自定义脏话词库到 JSON 文件"""
        try:
            # 获取文件路径，假设在 Main 类初始化时定义了 self.custom_swear_file
            file_path = getattr(self, 'custom_swear_file', None)
            if not file_path:
                # 如果未定义，使用默认路径
                file_path = os.path.join(os.path.dirname(__file__), "custom_swear_words.json")
                self.custom_swear_file = file_path

            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(self.custom_swear_words, f, ensure_ascii=False, indent=2)
        except Exception as e:
            logger.error(f"保存自定义脏话词库失败: {e}")

    # 提示：您需要在 Main 类的初始化方法中添加以下代码来加载词库：
    # def _init_swear_words(self):
    #     self.custom_swear_file = os.path.join(os.path.dirname(__file__), "custom_swear_words.json")
    #     self.custom_swear_words = []
    #     self.custom_swear_set = set()
    #     if os.path.exists(self.custom_swear_file):
    #         try:
    #             with open(self.custom_swear_file, 'r', encoding='utf-8') as f:
    #                 self.custom_swear_words = json.load(f)
    #                 self.custom_swear_set = set(self.custom_swear_words)
    #         except Exception as e:
    #             logger.warning(f"加载自定义脏话词库失败: {e}")
    #     
    #     # 如果使用了 AC 自动机，这里需要将自定义词库添加进去
    #     # 示例: for word in self.custom_swear_words: self._swear_matcher.add_word(word)
